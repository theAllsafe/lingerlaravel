...
use Validator;
class LibraryChapterController extends Controller
{
    ...
    public function index(){        
        $data = LibraryChapter::join('tbl_library_book','tbl_library_book.id','=','tbl_library_chapter.book_id')->select('tbl_library_chapter.*','tbl_library_book.book_name')->get();
    ...
    public function book(){        
        $data = LibraryBook::select('book_name as label','id as value')->get();
        // $columns[0]= ['Header'=>"sh.NO","accessor"=>'hindi'];
        if($data){
            return response(['success'=>true,'code'=> $this->successStatus,'message'=>'data found','data'=>$data],$this->successStatus);            
        }else{
    ...
    public function store(Request $request)
    {
        ...
        $add= new LibraryChapter;
        $add->uid=$request->input('uid'); 
        $add->book_id=$request->input('bid');
        $add->chapter_name=strip_tags($request->input('chapterName'));
        ...
        // $add->description=strip_tags($request->input('description'));
        $add->save();
        if($add){
   ...
   $file = $request->file('file');
    ...
    {
        $users=LibraryChapter::findOrFail($id);
        $users->delete();
