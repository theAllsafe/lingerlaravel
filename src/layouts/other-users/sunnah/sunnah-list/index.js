...
function ProductsList() {
  ...
  const fetchProducts = async () => {
    // setIsLoading(true);
    let result = await fetch(`${process.env.REACT_APP_DHEERAJ_KEY}admin/sunnah/`);
  ...
  const deleteRow = async (id) => {
    ...
    const result = await axios
      .delete(`${process.env.REACT_APP_DHEERAJ_KEY}admin/sunnah/destroy/${id}`)
      .then(({ data }) => {
        Swal.fire({
          icon: "success",
          text: data.message,
        ...
        <Card>
          <SuiBox display="flex" justifyContent="space-between" alignItems="flex-start" p={3}>
            ...
            <Stack spacing={1} direction="row">
              <Link to="/InformationSystem/Sunnah/sunnah" className="decoration-none">
