...
function ProductsList() {
  ...
  const fetchProducts = async () => {
    // setIsLoading(true);
    let result = await fetch(`${process.env.REACT_APP_DHEERAJ_KEY}admin/sukoon/`);
      ...
      if (row[name] === undefined) {
        ...
        template = (
          <SuiBox key={row[name]} component="td" p={1} textAlign={align}>
            <SuiBox display="flex" alignItems="center" py={0.5} px={1}>
              <SuiBox mr={2}>
                <Link to={row.audio} className="decoration-none">
        ...
        <Card>
          <SuiBox display="flex" justifyContent="space-between" alignItems="flex-start" p={3}>
            ...
            <Stack spacing={1} direction="row">
              <Link to="/InformationSystem/Sukoon/Sukoon" className="decoration-none">
                <SuiButton variant="gradient" buttonColor="info" size="small">
                  + Add new Sukoon
                </SuiButton>
              </Link>
