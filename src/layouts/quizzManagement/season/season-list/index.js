...
function ProductsList() {
  ...
  const fetchProducts = async () => {
    setIsLoading(true);
    let result = await fetch(`${process.env.REACT_APP_DHEERAJ_KEY}admin/season/`);
  ...
  const deleteRow = async (id) => {
    ...
    const result = await axios
      .delete(`${process.env.REACT_APP_DHEERAJ_KEY}admin/season/destroy/${id}`)
      .then(({ data }) => {
        Swal.fire({
          icon: "success",
          text: data.message,
            ...
            <SuiBox display="flex" alignItems="center" py={0.5} px={1}>
              <SuiBox mr={2}>
                <Tooltip title="Edit product" placement="top">
                  <Link
                    to={`/QuizzManagement/season/editseason/${row.id}`}
        ...
        <Card>
          <SuiBox display="flex" justifyContent="space-between" alignItems="flex-start" p={3}>
            ...
            <Stack spacing={1} direction="row">
              <Link to="/QuizzManagement/season/addseason" className="decoration-none">
          ...
          <TableContainer>
            <MuiTable>
              ...
              {!isLoading ? (
                ...
                <SuiBox component="thead">hello</SuiBox>
              )}
            </MuiTable>
          </TableContainer>
        </Card>
