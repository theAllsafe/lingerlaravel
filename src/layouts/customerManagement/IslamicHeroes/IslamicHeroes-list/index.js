...
function ProductsList() {
  ...
  const fetchProducts = async () => {
    // setIsLoading(true);
    let result = await fetch(`${process.env.REACT_APP_DHEERAJ_KEY}admin/islamicHeroes/`);
  ...
  const deleteRow = async (id) => {
    ...
    const result = await axios
      .delete(`${process.env.REACT_APP_DHEERAJ_KEY}admin/islamicHeroes/destroy/${id}`)
      .then(({ data }) => {
        Swal.fire({
          icon: "success",
          text: data.message,
        ...
        <Card>
          <SuiBox display="flex" justifyContent="space-between" alignItems="flex-start" p={3}>
            ...
            <Stack spacing={1} direction="row">
              <Link to="/Profile/IslamicHeroes/addIslamicHeroes" className="decoration-none">
                <SuiButton variant="gradient" buttonColor="info" size="small">
                  + Add new Islamic hero`s
                </SuiButton>
              </Link>
            </Stack>
          </SuiBox>
