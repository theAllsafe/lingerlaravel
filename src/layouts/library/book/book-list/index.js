...
function ProductsList() {
  const [columns] = useState([
    ...
    { name: "book_name", align: "center" },
    { name: "book_arabic_name", align: "center" },
  ...
  const fetchProducts = async () => {
    // setIsLoading(true);
    let result = await fetch(`${process.env.REACT_APP_DHEERAJ_KEY}admin/library/book/`);
  ...
  const deleteRow = async (id) => {
    ...
    const result = await axios
      .delete(`${process.env.REACT_APP_DHEERAJ_KEY}admin/library/book/destroy/${id}`)
      .then(({ data }) => {
        Swal.fire({
          icon: "success",
          text: data.message,
        ...
        <Card>
          <SuiBox display="flex" justifyContent="space-between" alignItems="flex-start" p={3}>
            ...
            <Stack spacing={1} direction="row">
              <Link to="/library/books/book" className="decoration-none">
