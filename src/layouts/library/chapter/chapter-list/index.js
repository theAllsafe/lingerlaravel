...
function ProductsList() {
  const [columns] = useState([
    ...
    { name: "chapter_name", align: "center" },
  ...
  const fetchProducts = async () => {
    // setIsLoading(true);
    let result = await fetch(`${process.env.REACT_APP_DHEERAJ_KEY}admin/library/chapter`);
  ...
  const deleteRow = async (id) => {
    ...
    const result = await axios
      .delete(`${process.env.REACT_APP_DHEERAJ_KEY}admin/library/chapter/destroy/${id}`)
      .then(({ data }) => {
        Swal.fire({
          icon: "success",
          text: data.message,
        ...
        <Card>
          <SuiBox display="flex" justifyContent="space-between" alignItems="flex-start" p={3}>
            ...
            <Stack spacing={1} direction="row">
              <Link to="/library/chapter" className="decoration-none">
